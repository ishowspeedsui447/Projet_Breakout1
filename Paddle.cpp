...
void Paddle::update(int windowWidth) {
	if (sf::Keyboard::isKeyPressed(sf::Keyboard::Left) && forme.getPosition().x - forme.getOrigin().x > 0)
	{
		forme.move(-mouvement, 0.f);
	}if (sf::Keyboard::isKeyPressed(sf::Keyboard::Right) && forme.getPosition().x + forme.getOrigin().x < windowWidth)
	{
		forme.move(mouvement, 0.f);
	}
}
